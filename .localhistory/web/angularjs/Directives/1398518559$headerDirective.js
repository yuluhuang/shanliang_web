...
shareDirectives.directive("headerD", function ($timeout, loginAPIService, searchAPIService, $location, $cookieStore) {
    return {
        ...
        link: function ($scope, $element, $attrs) {
            console.info("ddd", location.href.slice(-10));
            ...
            loginAPIService.islogin().success(function (data) {
                ...
                if (data[0].flag) {
                    ...
                    if ($location.path() != "/login" && $location.path() != "/index" && $location.path() != "/fastregister" && location.href.slice(-10) != "index.html" && location.href!= "http://yuluhuang.com") {
                        alert("qqqlogin");
                        $scope.islogining = false;
                        $scope.username = "";
                    }
